...
Promoted from the 742-line diagnostic script into the library so the
metric computation is unit-testable; the script is now a thin driver
around this module.
...
the audit is run and vary day to day.  The raw vs filtered runs are
...
1. yfinance + SPY (ETF) vs yfinance + ^SPX (index) changes the
   UNDERLYING — and with it the option market being measured — so any
   difference is an underlying effect, not a data-source effect.
2. OpenBB + SPY is configured with provider="yfinance", so OpenBB/SPY
   vs yfinance/SPY only tests the ingestion path (normalisation), NOT
   provider independence: the two paths share the same upstream provider.

For index symbols (^SPX), the per-expiry dividend yield is estimated
from put-call parity on the option chain itself (median across ATM
strikes), with a representative-ETF trailing yield (SPY for ^SPX) as
fallback when parity estimation fails.  q is NOT assumed to be zero.

For each source, runs with filter OFF (true baseline) and filter ON
(default thresholds: OI>=10, spread<=50%).  In fixture mode the
filtered comparison is N/A: the fixture is a post-ingestion surface
with no raw chains to filter.  Volume is recorded as diagnostic
context only — it is not a filter criterion.

The testable core: ``audit_surface`` (fixture-mode = offline),
``compute_*`` metric helpers, ``build_dip_comparison_lines`` and
``build_issues_markdown`` (N/A discipline — a source that was not
measured must never contribute a zero).  See ``tests/test_audit.py``.
"""
...
# ``src/arbfree_vol/data/audit.py`` sits three package levels below the
# repo root; the root-relative fixtures / docs files resolve through it.
...
def load_spx_fixture(path: Path | None = None) -> VolSurface:
    """Load SPX raw data from the saved fixture file.

    ``path`` defaults to ``tests/fixtures/spx_sample.json`` under the
    repo root.  The fixture is a POST-INGESTION surface (no raw chains),
    so per-expiry option-chain metrics are N/A in fixture mode.
    """
    import json
    ...
    fixture_path = path or (_repo_root() / "tests" / "fixtures" / "spx_sample.json")
...
def audit_surface(
    ...
    """Run the audit for ONE surface and return structured results (no prints).

    Pure computation — testable offline with a fixture surface (pass
    ``ticker=None``: per-expiry option-chain metrics then come back as
    N/A, never as observed zeros).  Rendering is separate:
    ``print_single_audit_report``.
    """
    ...
    return {
        ...
        "theta_mean_dip_pct": theta_info["mean_dip_pct"],
        "tenor_breakdown": compute_tenor_bucket_breakdown(surface, result.fallback_slices),
    }


def print_single_audit_report(label: str, surface, spot: float, result: dict) -> None:
    """Print one audit run's console report (header, table, tenor breakdown)."""
    print(f"\n{'=' * 72}")
    print(f"  {label}")
    print(f"{'=' * 72}")
    ...
    print(f"  Quality drops: {result['n_quality_drops']}")

    print(f"  Fitted: {result['n_fitted']} slices")
    ...
    print(f"  Theta dips: {result['theta_dips']}, "
          f"max={result['theta_max_dip_pct']:.1f}%, "
          f"mean={result['theta_mean_dip_pct']:.1f}%")
    ...
    _print_tenor_breakdown(result["tenor_breakdown"])
...
def compute_results(args) -> dict:
    ...
    spy_raw_result = audit_surface(
        "yfinance/SPY — FILTER OFF",
        spy_raw_surface, spy_raw_drops, spy_spot, ticker_spy,
    )
    print_single_audit_report("yfinance/SPY — FILTER OFF", spy_raw_surface, spy_spot, spy_raw_result)
    spy_filt_result = audit_surface(
        "yfinance/SPY — FILTER ON",
        spy_filt_surface, spy_filt_drops, spy_spot, ticker_spy,
    )
    print_single_audit_report("yfinance/SPY — FILTER ON", spy_filt_surface, spy_spot, spy_filt_result)
    ...
    try:
        ...
        if args.use_fixture:
            ...
            spx_filt_result = audit_surface(
                "yfinance/^SPX — FILTER ON",
                spx_filt_surface, spx_filt_drops, spx_spot, ticker_spx,
            )
            print_single_audit_report(
                "yfinance/^SPX — FILTER ON", spx_filt_surface, spx_spot, spx_filt_result,
            )
    ...
    try:
        ...
        if obb_raw_surface is not None:
            ...
            obb_raw_result = audit_surface(
                "OpenBB/SPY — FILTER OFF",
                obb_raw_surface, obb_raw_drops, obb_spot,
            )
            print_single_audit_report("OpenBB/SPY — FILTER OFF", obb_raw_surface, obb_spot, obb_raw_result)
            obb_filt_result = audit_surface(
                "OpenBB/SPY — FILTER ON",
                obb_filt_surface, obb_filt_drops, obb_spot,
            )
            print_single_audit_report("OpenBB/SPY — FILTER ON", obb_filt_surface, obb_spot, obb_filt_result)
            results["openbb_SPY"] = {
                "raw": obb_raw_result,
...
def build_dip_comparison_lines(results: dict) -> list[str]:
...
def build_issues_markdown(results: dict) -> str:
    """Build the Issue #15 section body (comparison table + conclusions).

    The returned string starts with a newline (mirroring the old
    ``lines`` list that began with an empty string) so section
    replacement in :func:`write_findings_to_issues` behaves identically.
    """
    lines = [
        "",
    ...
    lines.extend(build_dip_comparison_lines(results))
...
def write_findings_to_issues(results, issues_path: Path | None = None) -> None:
    """Update docs/issues.md Issue #15 with the underlying/path comparison.

    ``issues_path`` defaults to ``docs/issues.md`` under the repo root.
    Only the bounded comparison section is replaced (marker-driven), so
    later Issue #15 sections, including the determinism snapshot, are
    preserved.
    """
    target = issues_path or (_repo_root() / "docs" / "issues.md")
    ...
    if found_marker is not None:
        ...
        if next_section_idx == -1:
            ...
            new_data_source_section = build_issues_markdown(results) + "\n"
        ...
        content = content.rstrip() + "\n" + build_issues_markdown(results) + "\n"

    target.write_text(content, encoding="utf-8")
    print(f"\n  Wrote underlying/path comparison to {target}")