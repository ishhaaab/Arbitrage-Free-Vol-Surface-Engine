...
class SSVIStrategy:
    """Fit SSVI slices sequentially by increasing maturity.
    ...
    to the unconstrained fit (see ``CalibrationReport.fallback_slices``)
    do not satisfy even the necessary
    conditions.
    """
    name = "SSVI"
    ...
    def fit(self, cleaned_surface: VolSurface, fwd_curve: dict[float, float]) -> _PathFitResult:
        ...
        sorted_slices = sorted(cleaned_surface.slices, key=lambda sl: sl.expiry_time)
            ...
            # Sequential SSVI path (Hendriks & Martini 2019).
        ...
        if slices_data:
            ...
            for (sl, F), (T, pts) in zip(slice_meta, slices_data):
                ...
                if ssvi_params is None:
                    _logger.warning(
                        "SSVI: no fit for T=%.4f; skipping in fitted output",
                        T,
                    )
                    continue
