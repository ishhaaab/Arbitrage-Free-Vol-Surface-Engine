...
def price_space_dupire_sigma_loc(
    ...
    Computes sigma_loc^2 = [dC/dT + (r-q)K dC/dK + qC] / ((1/2) K^2 C_KK)
    directly from Black-Scholes call prices built on ``total_variance_at``
    — the repo's ACTUAL surface.  This is the model-independent definition
    of local volatility and does not use any total-variance-space formula,
    so it is a fully independent ground truth for the repo's surface.
    """
    import numpy as np
